...
const CalendarHeader: React.FC<CalendarHeaderProps> = ({
  ...
  const navButtonClasses =
    "text-sm text-gray-700 hover:text-gray-900 transition px-1";

  return (
    <div className="flex flex-wrap items-center gap-2 p-3 border-b border-[#dadce0]">
      <div className="flex items-center gap-2 flex-wrap">
        <button onClick={onToday} className={todayButtonClasses}>
          Today
        </button>
        <button onClick={onPrevMonth} className={navButtonClasses}>
          &lt;
        </button>
        <button onClick={onNextMonth} className={navButtonClasses}>
          &gt;
        </button>
